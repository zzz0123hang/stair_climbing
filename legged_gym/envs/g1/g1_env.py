...
class G1Robot(LeggedRobot):    
    ...
    def init_envs(self):
        ...
        # 取整并限制在地图范围内
        grid_x = grid_x.long().clamp(0, self.terrain.tot_cols - 1)
        grid_y = grid_y.long().clamp(0, self.terrain.tot_rows - 1)   
        # 6. 查表获取脚下的真实地形高度
    ...
    def _reward_stair_clearance(self):
        """
        [单步上台阶优化 - 避障奖励]
        鼓励摆动腿(Swing Leg)的脚尖在前进方向上高于地形。
        防止机器人为了省力贴地飞行，导致脚尖踢到台阶立面(Riser)。
        """
        # 1. 识别摆动腿 (非支撑相)
        is_swing = self.leg_phase > 0.55 # 或者是 contact < 1.0
        
        # 2. 预测脚尖前方一点的位置 (Lookahead)
        # 获取当前基座速度，判断前进方向
        lin_vel = self.base_lin_vel[:, :2]
        vel_norm = torch.norm(lin_vel, dim=1, keepdim=True)
        heading = lin_vel / (vel_norm + 1e-5) # 单位向量
        
        lookahead_dist = 0.05 # 向前看 5cm
        
        # 脚的位置
        feet_pos = self.feet_pos
        
        # 采样点：脚中心沿速度方向向前 0.05m
        ...
        # 3. 计算 Clearance
        # 我们希望：脚的高度 > 前方地形高度 + 安全余量 (比如 2cm)
        target_h = terrain_h + 0.02
        current_h = feet_pos[..., 2]
        
        # 误差：如果比目标低，就有惩罚
        error = target_h - current_h
        error = torch.clip(error, min=0.0) # 只惩罚低于目标的情况
        
        # 4. 只惩罚摆动腿且速度明显向前的时刻
        moving_mask = vel_norm > 0.1
        
        reward = torch.sum(torch.square(error) * is_swing.float(), dim=1)
        return reward * moving_mask.squeeze(1).float()
    ...
    def _reward_pelvis_height(self):
        ...
        # 获取支撑强度：利用你已有的 leg_phase (0~0.55为支撑) 
        # 或者直接用 contact_forces
        contact_forces = self.contact_forces[:, self.feet_indices, 2] # (num_envs, 2)
        weights = torch.clip(contact_forces / 10.0, 0.0, 1.0) # 受力越大，权重越高
        
        # 如果两脚都悬空（跳跃），给一个默认均值，防止 NaN
        sum_weights = torch.sum(weights, dim=1)
        
        # 计算加权平均高度
        weighted_foot_height = torch.sum(feet_heights * weights, dim=1) / (sum_weights + 1e-4)
        
        # 只有当至少有一只脚踩地时才计算该奖励
        dist = base_height - weighted_foot_height
        target_height = 0.70
        
        reward = torch.square(dist - target_height)
        return reward * (sum_weights > 0.1).float() # 没踩地时不惩罚高度
